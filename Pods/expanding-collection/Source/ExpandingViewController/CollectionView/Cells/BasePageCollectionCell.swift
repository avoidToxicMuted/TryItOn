...
extension BasePageCollectionCell {
    ...
    public func cellIsOpen(_ isOpen: Bool, animated: Bool = true) {
        ...
        if ySpacing == .greatestFiniteMagnitude {
            frontConstraintY.constant = isOpen == true ? -frontContainerView.bounds.size.height / 6 : 40
            backConstraintY.constant = isOpen == true ? frontContainerView.bounds.size.height / 6 - yOffset / 2 : 40
        } else {
            frontConstraintY.constant = isOpen == true ? -ySpacing / 2 : 40
            backConstraintY.constant = isOpen == true ? ySpacing / 2 : 40
        }
...
extension BasePageCollectionCell {
    ...
    fileprivate func configurationViews() {
        ...
        frontConstraintY.constant = 40
        backConstraintY.constant = 40 
        
        backContainerView.layer.masksToBounds = true
        backContainerView.layer.cornerRadius = 12.0

        frontContainerView.layer.masksToBounds = true
        frontContainerView.layer.cornerRadius = 12.0
